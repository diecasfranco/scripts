...
if type_of_data == "COI":
   ...
   os.system("""for file in *.fasta; do
    ...
    os.system("""for file in *.fasta; do
        SampleName=`basename $file .fasta`
        egrep -B 1 "CCTACGGG[ACTG]GGC[ATCG]GCAG.{400,430}GGATTAGATACCC[ATCG][ATCG]GTAGTC" $SampleName.fasta | egrep -v "^\-\-$" |
        sed -E 's/.*CCTACGGG[ACTG]GGC[ATCG]GCAG//; s/GGATTAGATACCC[ATCG][ATCG]GTAGTC.*//' > "$SampleName".trimmed.fasta
    done""")
