...
print("THIS SCRIPT WAS PREPARED FOR IGOR AND ZUZA <3")
if len(sys.argv) != 4:
    sys.exit("""ERROR! CHECK YOUR INPUT PARAMETERS! [version 20231126_2051]
...
3) <data_type> - the type of target data. Currently, options include the 16SV4, 16SV12 (Bacterial), ITS2, ITS1a, 18SV78 (Fungal) or COI (or COInem).""")
...
for line in SAMPLE_LIST:
    ...
    if type_of_data == "COI":
        ...
        os.system("pear -f %s -r %s -o %s -v 15 -n 250 -q 30 -j 20" % (LINE[1], LINE[2], LINE[0]))
    elif type_of_data == "18SV78":
        os.system("pear -f %s -r %s -o %s -v 15 -n 250 -m 400 -q 30 -j 55" % (LINE[1], LINE[2], LINE[0]))
    elif type_of_data == "16SV12":
        os.system("pear -f %s -r %s -o %s -v 15 -n 250 -m 400 -q 30 -j 55" % (LINE[1], LINE[2], LINE[0]))
    #elif type_of_data == "16SV34":
    #    os.system("pear -f %s -r %s -o %s -v 15 -n 250 -m 470 -q 30 -j 60" % (LINE[1], LINE[2], LINE[0]))
...
if type_of_data == "COI":
    os.system ("mv *_R?_CO*.fastq reads/")
elif type_of_data == "16SV4":
    os.system ("mv *_R?*.fastq reads/")
elif type_of_data == "16SV12":
    os.system ("mv *_R?_16SV12.fastq reads/")
elif type_of_data == "ITS2":
    os.system ("mv *_R?_ITS2.fastq reads/")
elif type_of_data == "ITS1a":
    os.system ("mv *_R?_ITS1a.fastq reads/")
elif type_of_data == "18SV78":
    os.system ("mv *_R?_18SV78.fastq reads/")
else:
    sys.exit("ERROR! Your specified data type was not in the list!")
...
elif type_of_data in ["16SV4", "16SV12", "18SV78"]:
    os.system("""for file in *sorted.fasta; do
        SampleName=`basename $file .sorted.fasta`
        usearch -unoise3 $SampleName.sorted.fasta -zotus $SampleName.zotus.fasta -tabbedout $SampleName.denoising.summary.txt -minsize 1
    done""")
elif type_of_data in ["ITS2", "ITS1a"]:
    os.system("""for file in *sorted.fasta; do
        SampleName=`basename $file .sorted.fasta`
        usearch -unoise3 $SampleName.sorted.fasta -zotus $SampleName.zotus.fasta -tabbedout $SampleName.denoising.summary.txt -minsize 2
    done""") 
...
os.system("""for file in *.fasta; do
    ...
    usearch -otutab ./raw_fasta/"$SampleName"_raw.fasta -zotus $SampleName.zotus.fasta -otutabout "$SampleName"_zotu_table.txt -threads 24
done""")
print("OK!")
...
os.system("""for file in *.fasta; do
    SampleName=`basename $file .zotus.fasta`
    /home/diego/scripts/add_seq_to_zotu.py "$SampleName"_zotu_table.txt "$SampleName".zotus.fasta "$SampleName"_zotu_table_with_seq.txt
done""")
...
vsearch --sintax otus.fasta -db /home/diego/databases/SILVA_endo_spikeins_RDP.fasta -tabbedout otus.tax -strand both -sintax_cutoff 0.8""")
    ...
    os.system("""vsearch --sintax new_zotus.fasta -db /mnt/qnap/users/symbio/software/databases/unite_fungi.fasta -tabbedout zotus.tax -strand both -sintax_cutoff 0.8
vsearch --sintax otus.fasta -db /mnt/qnap/users/symbio/software/databases/unite_fungi.fasta -tabbedout otus.tax -strand both -sintax_cutoff 0.8""")    
