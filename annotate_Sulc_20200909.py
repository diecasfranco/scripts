...
work_dir = "/home/junchen.deng/annotation_Vidania/round9_DIC/"
#work_dir = "/home/junchen.deng/annotation_Sulcia/round11_DIC/"

  ### Folder with genomes to annotate. Any fasta files in that dir will be regarded as genomes for annotation
genomes_for_annotation_dir = "/home/junchen.deng/annotation_Vidania/genomes_for_annotation/"
#genomes_for_annotation_dir = "/home/junchen.deng/annotation_Sulcia/genomes_for_annotation/"
...
protein_ref_dir = "/home/junchen.deng/annotation_Vidania/proteins/"
#protein_ref_dir = "/home/junchen.deng/annotation_Sulcia/Sulcia_annotation_references/proteins_20210719/"


  ### Folder containing alignments of all rRNA, tmRNA and ncRNA genes to be searched. File names: gene.fasta
rRNA_ref_dir = "/home/junchen.deng/annotation_Vidania/RNAs/"
#rRNA_ref_dir = "/home/junchen.deng/annotation_Sulcia/Sulcia_annotation_references/RNAs/"

  ### Table with detailed information about every gene: name, EC number, etc.
gene_detail_table = "/home/junchen.deng/annotation_Vidania/Vidania_gene_list.txt"
#gene_detail_table = "/home/junchen.deng/annotation_Sulcia/Sulcia_gene_list.txt"

Threshold_functional = 0.6
Threshold_putative = 0.6
Upper_threshold_functional = 1.5
Size_reference = "VFOLIH"
#Size_reference = "SULCIA_ALF"
Translation_table = 11	### As defined by getorf / NCBI!
						### 4: Spiroplasma, Hodgkinia, Nasuia; 11: Sulcia, bacteria; 5: Mitogenome
...
if Translation_table == 11:   ### Sulcia, bacteria
    ...
    start_codon_list = ['ATT', 'ATG', 'ATA','GTG', 'TTG', 'CTG']    # Vidania
    #start_codon_list = ['ATG', 'ATA','GTG', 'TTG', 'CTG']    # Sulcia
    #start_codon_list = ['ATG','GTG', 'TTG', 'CTG']    # Sulcia
    stop_codon_list = ['TAA', 'TAG', 'TGA']
elif Translation_table == 4:   ### Spiroplasma, Hodgkinia, Nasuia 
    #start_codon_list = ['ATA','ATT','ATG', 'ATC', 'GTG', 'TTG', 'CTG','TTA']
    start_codon_list = ['ATG', 'ATC', 'GTG', 'TTG', 'CTG']
    stop_codon_list = ['TAA', 'TAG']
    ...
    for filename in glob.glob("%s/*_prot.fasta" % protein_ref_dir):
        ...
        for i in range(len(PROT_FASTA)):
            if Size_reference in PROT_FASTA[i][0]:
                ref_pos = i    # save the position of size_reference [default: 0]
                break
        ...
        for aa in seq[1]:
            if aa != "-":
                new_sequence += aa    # remove "-" in the protein seq
        seq[1] = new_sequence
        seq[0] = gene_name
        ...
        for k in Gene_range:
            ...
            for line in HITS:   
                if not line.startswith('#'):
                    Line =  line.split()
                    Hit_list.append(Line[0])    # Line[0] = genomeX ORF name; Line[3] = hit name, e.g. "SMOLIH_aceF", Line[6] = E-value
                    if Line[0] not in ORFs_used:
                        ORFs_used[Line[0]] = [[Line[3].split("_")[0], Line[6]]]
                    else:
                        ORFs_used[Line[0]].append([Line[3].split("_")[0], Line[6]])    # save more than one hits
            HITS.close()
            ...
            if len(Hit_list) == 0:
                ...
                if not glob.glob(ali_name):
                    ...
                    for ORF in ORF_list:
                        if ORF[0].split()[0] in Hit_list:    # find the ORFs that have the hits
                ...
                if len(Hit_table) > 0:   ### Look at first (best) hit. If long enough to be classified as PUTATIVE or FUNCTIONAL, don't look at others.
                    if  Hit_table[0][6] == 'F':
                        Hit_start = int(Hit_table[0][4])+(int(Hit_table[0][2]))*3    # account for the mismatches at the beginning of the alignment
            ...
            if appendix[0] in ['Functional', 'Putative']:
                ...
                if appendix[6] == 'F':
                    ...
                    ORF_start = appendix[2]    # appendix[2] = start_in_genome
                    Updated_start = 0
                    if GENOME[0][1][Annotation_start-1:Annotation_start+2] in start_codon_list:
                        ...
                        if not Updated_start:     #### If there was no alternative start codon before the original start position...
                            Test_start = Annotation_start
                            while Test_start <= appendix[3]:    # appendix[3] = end_in_genome
                                if GENOME[0][1][Test_start-1:Test_start+2] in start_codon_list:
