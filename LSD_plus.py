...
4) number of cores for the process.""")
Script, sample_list, path_to_your_raw_data, type_of_data, num_cores = sys.argv
...
for line in SAMPLE_LIST:
    ...
    if type_of_data == "COI":
        os.system("pear -f %s -r %s -o %s -v 15 -n 400 -m 470 -q 30 -j %s" % (LINE[1], LINE[2], LINE[0], num_cores))
    elif type_of_data == "16SV4":
        os.system("pear -f %s -r %s -o %s -v 15 -n 250 -q 30 -j %s" % (LINE[1], LINE[2], LINE[0], num_cores))
    elif type_of_data == "16SV1-V2":
        os.system("pear -f %s -r %s -o %s -v 15 -n 250 -m 400 -q 30 -j %s" % (LINE[1], LINE[2], LINE[0], num_cores))
...
os.system("rename .fasta _raw.fasta *fasta ")
...
os.system("""for file in *.fasta; do
    ...
    usearch -otutab ./raw_fasta/"$SampleName"_raw.fasta -zotus "$SampleName".zotus.fasta -otutabout "$SampleName"_zotu_table.txt -threads {num_cores}
done""")
print("OK!")
...
os.system("usearch -cluster_otus zotus.fasta -otus otus.fasta -relabel OTU -uparseout zotu_otu_relationships.txt -threads {num_cores}")
print("OK!") 
...
if type_of_data == "COI":
    os.system("""vsearch --sintax new_zotus.fasta -db /mnt/qnap/users/symbio/software/databasesMIDORI_with_tax_spikeins_endo_RDP.fasta -tabbedout zotus.tax -strand both -sintax_cutoff 0.8
...
print("Symbio® Na zdrowie! Salud! Gānbēi (干杯)! Skål! Sante - goed bezig!")
