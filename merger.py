    ...
    for line in Table:
        ...
        if use_full_db_format:
            ...
            if include_full_name:
                full_name = LINE[16]  # Use the last column for full name
        else:
...
for row in range(1, row_count):
    if Table_cov[row][0] in Table_tax.keys():
        ...
        if include_full_name:
            Table_cov[row].extend([taxonomy, kingdom, full_name])
        else:
            Table_cov[row].extend([taxonomy, kingdom])
    else:
        if include_full_name:
            Table_cov[row].extend(["NA", "NA", "NA"])  # Set both taxonomy, kingdom and full_name to "NA" if not found
        else:
            Table_cov[row].extend(["NA", "NA"])  # Set both taxonomy and kingdom to "NA" if not found
...
# Add the full name to the final table if "full_name" is set
if include_full_name:
    Table_cov[0].append("full_name")  # Header for the new column
    for row in range(1, row_count):
        if Table_cov[row][0] in Table_tax.keys():
