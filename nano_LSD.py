...
2) path to the directory with FASTA files for all the amplicon libraries that you want to analyse e.g.:
...
os.system("mkdir raw_fasta && mv *.fasta raw_fasta/")
...
if type_of_data in ["COI", "16SV4", "16SV1-V2"]:
...
os.system("""for file in *.zotus.fasta; do
    SampleName=`basename $file .zotus.fasta`
    usearch -otutab ./raw_fasta/"$SampleName".fasta -zotus "$SampleName".zotus.fasta -otutabout "$SampleName"_zotu_table.txt -threads {num_cores}
done""")
print("OK!")
...
os.system(f"usearch -cluster_otus zotus.fasta -otus otus.fasta -relabel OTU -uparseout zotu_otu_relationships.txt -threads {num_cores}")
print("OK!") 
...
elif type_of_data in ["16SV4", "16SV1-V2"]:
